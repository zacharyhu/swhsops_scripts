...
if [ -e $LOGFILE ];then
  ...
  awk '{print $4,$7}' ${LOGFILE} |grep '/gate_xj/gameAdd' | awk -F '[/|=|&]' '{print $6}' |sort |uniq -c |sort -nr|awk '{print DATE"|105|"$1"|"$2}' DATE=$DATE  >>$gamefile
  awk '{print $4,$7}' ${LOGFILE} |grep '/gate_igw/gameAdd' | awk -F '[/|=|&]' '{print $6}' |sort |uniq -c |sort -nr|awk '{print DATE"|104|"$1"|"$2}' DATE=$DATE  >>$gamefile
  awk '{print $4,$7}' ${LOGFILE} |grep '/gate_tz/gameAdd' | awk -F '[/|=|&]' '{print $6}' |sort |uniq -c |sort -nr|awk '{print DATE"|104|"$1"|"$2}' DATE=$DATE  >>$gamefile
