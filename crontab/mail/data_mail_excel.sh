...
cpid=(105 104 501 201 502 503 204 207 205 202 504)
...
echo "select l_game_points from gp_platform.gp_data_daily_game where l_source=0 and l_date=DATE_FORMAT(DATE_SUB(CURDATE(), INTERVAL 1 DAY),'%y%m%d')"|$myquery >>$LOGFILE

for ((i=0;i<${#gameid[*]};i++))
do
 echo "select sum(l_game_points),sum(l_user_num) from gp_platform.gp_data_daily_game where l_game_id=${gameid[$i]}  and l_date=DATE_FORMAT(DATE_SUB(CURDATE(), INTERVAL 1 DAY),'%y%m%d')" |$myquery  >>$LOGFILE
done
